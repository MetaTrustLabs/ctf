...
fn handle_client(mut stream: TcpStream) -> Result<(), Box<dyn Error>> {
    ...
    let arg_ob = SuiValue::Object(FakeID::Enumerated(1, 0));
    ...
    match ret_val {
        Ok(()) => {
            ...
            if let Ok(flag) = env::var("FLAG") {
                ...
                stream
                    .write("[SERVER] Flag not found, please contact admin".as_bytes())
                    .unwrap();
            }
        }
        Err(_error) => {
            ...
            println!("");
            stream
                .write("[SERVER] Invalid Solution!".as_bytes())
                .unwrap();
        }
    };
